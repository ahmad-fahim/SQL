CREATE OR REPLACE PACKAGE PKG_LOANDAILYACCRPOST_MRR
IS
   PROCEDURE SP_LOANACCRPOST (V_ENTITY_NUM IN NUMBER, P_CALL_FROM CHAR, P_BRN_CODE NUMBER, P_INTERNAL_ACNUM NUMBER );

   TYPE AMOUNT_FORMAT IS RECORD (ROUND_DIGIT NUMBER (2));

   TYPE TT_AMOUNT_FORMAT IS TABLE OF AMOUNT_FORMAT
      INDEX BY VARCHAR2 (100);

   T_AMOUNT_FORMAT   TT_AMOUNT_FORMAT;

   FUNCTION FN_GET_AMOUNT_FORMAT (P_ENTITY_NUMBER   IN NUMBER,
                                  P_CURR_CODE          VARCHAR2,
                                  P_AMOUNT             NUMBER)
      RETURN NUMBER;
END PKG_LOANDAILYACCRPOST_MRR;
/
CREATE OR REPLACE PACKAGE BODY PKG_LOANDAILYACCRPOST_MRR
IS
   V_GLOB_ENTITY_NUM                NUMBER;
   V_CALL_FROM_MRR_REVERSAL           VARCHAR2(1);
   ...
   W_CALL_FROM                      CHAR (1); -- Note: 1 = Clouser,Reversal , 2 = EoD Process (Mirror)



   TYPE TLOANIA_ACNT_NUM IS TABLE OF NUMBER (14)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_VALUE_DATE IS TABLE OF DATE
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_MAX_VALUE_DATE IS TABLE OF DATE
      INDEX BY PLS_INTEGER;                                           -- Note:

   TYPE TLOANIA_ACNT_CURR IS TABLE OF VARCHAR (3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_ACNT_BAL IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_PREV_ACCR_DATE IS TABLE OF DATE
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_INT_ON_AMT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_OD_PORTION IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_TOTAL_NEW_INT_AMT IS TABLE OF NUMBER (18, 9)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_TOTAL_NEW_OD_INT_AMT IS TABLE OF NUMBER (18, 9)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_INT_RATE IS TABLE OF NUMBER (8, 5)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_SLAB_AMT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_OD_INT_RATE IS TABLE OF NUMBER (8, 5)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_LIMIT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_DP IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_INT_AMT IS TABLE OF NUMBER (18, 9)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_INT_AMT_RND IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_OD_INT_AMT IS TABLE OF NUMBER (18, 9)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_OD_INT_AMT_RND IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_NPA_INT_POSTED_AMT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   --09-08-2010-beg
   TYPE TLOANIA_ARR_INT_AMT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   --09-08-2010-end

   TYPE TLOANIA_NPA_STATUS IS TABLE OF CHAR (1)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIA_NPA_AMOUNT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_ACNT_NUM IS TABLE OF NUMBER (14)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_VALUE_DATE IS TABLE OF DATE
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_ACCRUAL_DATE IS TABLE OF DATE
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_SL_NUM IS TABLE OF NUMBER (5)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_INT_RATE IS TABLE OF NUMBER (8, 5)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_UPTO_AMT IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_INT_AMT IS TABLE OF NUMBER (18, 9)
      INDEX BY PLS_INTEGER;

   TYPE TLOANIADTL_INT_AMT_RND IS TABLE OF NUMBER (18, 3)
      INDEX BY PLS_INTEGER;
   ...
   (
      ...
      RTMPLNIA_MAX_VALUE_DATE          DATE,
      W_INT_ACCR_UPTO_DATE             DATE,
      W_AC_BRN_CODE                    MBRN.MBRN_CODE%TYPE,
      W_AC_CURR_CODE                   ACNTS.ACNTS_CURR_CODE%TYPE,
      W_AC_PROD_CODE                   ACNTS.ACNTS_PROD_CODE%TYPE,
      W_LNACNT_PA_ACCR_POSTED_UPTO     LOANACNTS.LNACNT_PA_ACCR_POSTED_UPTO%TYPE,
      W_LNACNT_INT_APPLIED_UPTO_DATE   LOANACNTS.LNACNT_INT_APPLIED_UPTO_DATE%TYPE
   );
   ...
   TYPE TLOANCALCPOSTRECORD IS TABLE OF LOANCALCPOSTRECORD
   ...
   TYPE TLOANCALCODPOSTRECORD IS TABLE OF LOANCALCODPOSTRECORD
      INDEX BY VARCHAR2 (14);
   ...
   TYPE TLOANCALCNPAPOSTRECORD IS TABLE OF LOANCALCNPAPOSTRECORD
      INDEX BY VARCHAR2 (14);
   ...
   TYPE TLOANCALCODNPAPOSTRECORD IS TABLE OF LOANCALCODNPAPOSTRECORD
      INDEX BY VARCHAR2 (14);
   ...
   TYPE T_ACWISE_NPA_INT IS TABLE OF ACWISE_NPA_INT
      INDEX BY PLS_INTEGER;
   ...
   TYPE TRTMPLNIA_RECORD IS TABLE OF RTMPLNIA_RECORD
      INDEX BY PLS_INTEGER;
   ...
   TYPE TLOAN_PROD_CURR_GL IS TABLE OF LOAN_PROD_CURR_GL
      INDEX BY VARCHAR2 (7);

   MLOAN_PROD_CURR_GL               TLOAN_PROD_CURR_GL;

   TYPE TLNACNT_PA_ACCR_POSTED_UPTO IS TABLE OF DATE
      INDEX BY PLS_INTEGER;
   ...
   MLOANIA_MAX_VALUE_DATE           TLOANIA_MAX_VALUE_DATE;           -- Note:
   ...
   MLOANIADTL_ACCRUAL_DATE          TLOANIADTL_ACCRUAL_DATE;
   ...
   BEGIN
      ...
      IF T_AMOUNT_FORMAT.EXISTS (V_SERCH_KEY) = TRUE
      THEN
         V_ROUND_DIGIT := T_AMOUNT_FORMAT (V_SERCH_KEY).ROUND_DIGIT;

         IF V_SBUK_BRANCH = TRUE
         THEN
            RETURN TRUNC (TO_NUMBER (P_AMOUNT), 2);
         ELSE
            RETURN TRUNC (ROUND (P_AMOUNT, V_ROUND_DIGIT - 1),
                          V_ROUND_DIGIT - 1);
         END IF;
      END IF;

      IF TRIM (P_CURR_CODE) <> 'INR'
      THEN
        <<READCURRENCY>>
         BEGIN
      ...
      BEGIN
         SELECT CBS_IMP_KEY_2
           INTO W_BANK_CODE1
           FROM CBS_IMP
          WHERE     CBS_IMP_ENTITY_NUM = 1
                AND CBS_IMP_BRN_CODE = 999999
                AND CBS_IMP_MODULE_ID = 'TRAN'
                AND CBS_IMP_KEY_1 = 'BASECONVERSION'
                AND CBS_IMP_VALUE = 1;
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
      ...
      ELSE
         ...
         RETURN TRUNC (ROUND (P_AMOUNT, LENGTH (W_SUB_UNITS) - 1),
                       LENGTH (W_SUB_UNITS) - 1);
      END IF;
   END;


   PROCEDURE CLEAR_LOANIAARRAY
   IS
   BEGIN
      MLNACNT_PA_ACCR_POSTED_UPTO.DELETE;
      MLOANIA_ACNT_NUM.DELETE;
      MLOANIA_VALUE_DATE.DELETE;
      MLOANIA_MAX_VALUE_DATE.DELETE; -- Note: Accrual Date Will update with the Max date of LOANIAMRR Table.
   ...
   PROCEDURE CLEAR_LOANIADTLARRAY
   IS
   BEGIN
      MLOANIADTL_ACNT_NUM.DELETE;
      MLOANIADTL_VALUE_DATE.DELETE;
      MLOANIADTL_ACCRUAL_DATE.DELETE;
   ...
   PROCEDURE CLEAR_AUTOPOST_ARRAY
   IS
   BEGIN
   ...
   PROCEDURE POPULATE_LOANACCR_GL
   IS
      --16-08-2010-beg
      ...
      TYPE TLOAN_PROD_CURR_GL IS TABLE OF TEMP_LOAN_PROD_CURR_GL
         INDEX BY PLS_INTEGER;
   ...
   BEGIN
      SELECT L.LNPRDAC_PROD_CODE,
             ...
             L.LNPRDAC_ACCRINT_SUSP_HEAD
        BULK COLLECT INTO MTEMPLOAN_PROD_CURR_GL
        FROM LNPRODACPM L;

      IF MTEMPLOAN_PROD_CURR_GL.COUNT > 0
      THEN
         FOR IDX IN 1 .. MTEMPLOAN_PROD_CURR_GL.COUNT
         LOOP
           --16-08-2010-beg
           <<READPRODPM>>
            BEGIN
               ...
               SELECT NVL (L.LNPRD_UNREAL_INT_INCOME_REQD, 0), -- NEELS-MDS-08-NOV-2010
                ...
                WHERE L.LNPRD_PROD_CODE =
                         MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CODE;
            EXCEPTION
               WHEN NO_DATA_FOUND
               THEN
                  P_LNPRD_UNREAL_INT_INCOME_REQD := '0'; -- NEELS-MDS-30-NOV-2010 ADD
            ...
            IF P_LNPRD_UNREAL_INT_INCOME_REQD = '1'
            THEN
               -- NEELS-MDS-30-NOV-2010
               MLOAN_PROD_CURR_GL (
                  MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_INT_INCOME_GL :=
                  NVL (
                     MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_ACCRINT_SUSP_HEAD,
                     '');
            ELSE
               MLOAN_PROD_CURR_GL (
                  MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_INT_INCOME_GL :=
                  MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_INT_INCOME_GL;
            END IF;

            MLOAN_PROD_CURR_GL (
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_INT_ACCR_GL :=
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_INT_ACCR_GL;

            MLOAN_PROD_CURR_GL (
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_PENAL_OD_INT_GL :=
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PENAL_OD_INT_GL;

            MLOAN_PROD_CURR_GL (
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_LOAN_PAST_DUE_GL :=
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_LOAN_PAST_DUE_GL;

            MLOAN_PROD_CURR_GL (
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_INT_SUSP_GL :=
               MTEMPLOAN_PROD_CURR_GL (IDX).LNPRDAC_INT_SUSP_GL;
         END LOOP;
      END IF;
   ...
   PROCEDURE GET_LOANIA_NPA_INT_AMOUNT
   IS
     ...
     <<READLNPRODPM>>
      BEGIN
         SELECT L.LNPRD_INT_APPL_FREQ
           ...
           FROM LNPRODPM L
          WHERE L.LNPRD_PROD_CODE =
                   M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_PROD_CODE;
      EXCEPTION
         WHEN OTHERS
         THEN
            W_APPL_ON_INT_RECOVERY := '';
      ...
      IF NVL (W_APPL_ON_INT_RECOVERY, '') = 'I'
      THEN
         W_I_BASED_LOAN := TRUE;
      ...
      THEN
         ...
         SELECT NVL (SUM (L.LOANIA_INT_AMT), 0),
          ...
          WHERE     LOANIA_ENTITY_NUM = V_GLOB_ENTITY_NUM
                AND L.LOANIA_BRN_CODE =
                       M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_BRN_CODE
                AND L.LOANIA_ACNT_NUM =
                       M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM
                AND                                           --14-10-2008-rem
                   L.LOANIA_VALUE_DATE > W_LNACNT_NPA_POSTED_DATE
                AND L.LOANIA_VALUE_DATE <= W_PROCESS_DATE
                AND L.LOANIA_NPA_STATUS = 1;
      END IF;

      -- Note: This Method Will Remove Later.
      W_NEW_MLOANIA_INT_AMT := 0;
      W_NEW_MLOANIA_INT_AMT_RND := 0;
   END GET_LOANIA_NPA_INT_AMOUNT;

   PROCEDURE PROCESS_FOR_ACTUAL_INTEREST
   IS
   BEGIN
      W_NEW_MLOANIA_INT_AMT := 0;
      W_NEW_MLOANIA_INT_AMT_RND := 0;
      ...
      MLOANIA_TOTAL_NEW_INT_AMT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_INT_AMT;
      MLOANIA_TOTAL_NEW_OD_INT_AMT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_OD_INT_AMT;

      IF W_PREV_ROW_AVL = TRUE
      THEN
         W_OLD_TOTAL_INT_AMOUNT := V_LOANIA_PREV_ROW.LOANIA_TOTAL_NEW_INT_AMT;
         W_OLD_TOTAL_INT_OD_AMOUNT :=
            V_LOANIA_PREV_ROW.LOANIA_TOTAL_NEW_OD_INT_AMT;
      ...
      IF W_PREV_ROW_AVL = TRUE
      THEN
         MLOANIA_PREV_ACCR_DATE (W_BRN_INDEX) :=
            V_LOANIA_PREV_ROW.LOANIA_ACCRUAL_DATE;
      ELSE
      ...
      MLOANIA_INT_AMT_RND (W_BRN_INDEX) :=
           SIGN (MLOANIA_INT_AMT (W_BRN_INDEX))
         * ABS (
              FN_GET_AMOUNT_FORMAT (
                 V_GLOB_ENTITY_NUM,
                 M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_CURR_CODE,
                 MLOANIA_INT_AMT (W_BRN_INDEX)));

      --MLOANIA_INT_ON_AMT (W_BRN_INDEX) :=
      --   MLOANIA_ACNT_BAL (W_BRN_INDEX) - W_OLD_TOTAL_AMOUNT;
      ...
      MLOANIA_OD_INT_AMT_RND (W_BRN_INDEX) :=
           SIGN (MLOANIA_OD_INT_AMT (W_BRN_INDEX))
         * ABS (
              FN_GET_AMOUNT_FORMAT (
                 V_GLOB_ENTITY_NUM,
                 M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_CURR_CODE,
      ...
      MLOANIA_NPA_STATUS (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_NPA_STATUS;

      MLOANIA_NPA_AMOUNT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_NPA_AMT;

      -- Note: Dtl table Updation.
      IF W_CALL_FROM = '1'
      THEN
         --INSERT_INTO_LOANIADTL;   --changed in 13/04/2017

         MLOANIADTL_ACNT_NUM (W_BRN_INDEX) :=
            M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM;

         MLOANIADTL_VALUE_DATE (W_BRN_INDEX) :=
            M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_VALUE_DATE;

         MLOANIADTL_ACCRUAL_DATE (W_BRN_INDEX) :=
            M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACCRUAL_DATE;

         MLOANIADTL_SL_NUM (W_BRN_INDEX) := 1;

         MLOANIADTL_INT_RATE (W_BRN_INDEX) :=
            M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_INT_RATE;

         MLOANIADTL_UPTO_AMT (W_BRN_INDEX) :=
            MLOANIA_ACNT_BAL (W_BRN_INDEX) - W_OLD_TOTAL_AMOUNT;

         MLOANIADTL_INT_AMT (W_BRN_INDEX) :=
              M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_INT_AMT;
         --   - W_OLD_TOTAL_INT_AMOUNT;

         MLOANIADTL_INT_AMT_RND (W_BRN_INDEX) :=
              SIGN (MLOANIA_INT_AMT (W_BRN_INDEX))
            * ABS (
                 FN_GET_AMOUNT_FORMAT (
                    V_GLOB_ENTITY_NUM,
      ...
      ELSIF W_CALL_FROM = '2'
      THEN
         -- INSERT_INTO_LOANIAMRRDTL;   --changed in 13/04/2017
		 NULL ;
      END IF;

      W_PROD_CURR_KEY :=
            LPAD (M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_PROD_CODE,
                  4,
                  0)
         || M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_CURR_CODE;

      IF W_NPA_PREV_ACCOUNT_NUMBER <>
            M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM
      THEN
      ...
      IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 1
      THEN
      ...
      W_NPA_PREV_ACCOUNT_NUMBER :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM;

      MLOANIA_NPA_INT_POSTED_AMT (W_BRN_INDEX) := 0;

      IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 0
      THEN
         W_LNACNT_NPA_POSTED_DATE := NULL;

         IF W_LNACNT_PA_ACCR_POSTED_UPTO IS NOT NULL
         THEN
            IF W_LNACNT_INT_APPLIED_UPTO_DATE IS NOT NULL
            THEN
               IF W_LNACNT_PA_ACCR_POSTED_UPTO >
      ...
      MLNACNT_PA_ACCR_POSTED_UPTO (W_BRN_INDEX) :=
         W_LNACNT_PA_ACCR_POSTED_UPTO;

      -- Note: This Portion I dont think is necessary... Because Last Date it Backtrack the Previous Accrual amount, Need to Discuss
      IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 0
      THEN
         MLNACNT_PA_ACCR_POSTED_UPTO (W_BRN_INDEX) := W_PROCESS_DATE;

         IF M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL) --14-11-2008-rem                .RTMPLNIA_VALUE_DATE = PKG_PB_GLOBAL.FN_GET_CURR_BUS_DATE THEN
                                                  .RTMPLNIA_VALUE_DATE =
               W_PROCESS_DATE
         THEN
            IF     W_LNACNT_NPA_POSTED_DATE IS NOT NULL
               AND (W_LNACNT_NPA_POSTED_DATE <> W_INT_ACCR_UPTO_DATE)
            THEN
            ...
            MLOANIA_NPA_INT_POSTED_AMT (W_BRN_INDEX) :=
               W_NEW_MLOANIA_INT_AMT_RND + W_NPA_INT_POST_AMOUNT_RND;
         END IF;
      END IF;

      IF MLOANIA_INT_AMT (W_BRN_INDEX) <> 0
      THEN
         IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 0
         THEN
            IF M_LOANCALCPOSTRECORD.EXISTS (W_PROD_CURR_KEY) = TRUE
            THEN
               M_LOANCALCPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT :=
                    M_LOANCALCPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT
                  + MLOANIA_INT_AMT_RND (W_BRN_INDEX);
            ELSE
               M_LOANCALCPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT :=
      ...
      IF MLOANIA_OD_INT_AMT (W_BRN_INDEX) <> 0
      THEN
         IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 0
         THEN
            IF M_LOANCALCODPOSTRECORD.EXISTS (W_PROD_CURR_KEY) = TRUE
            THEN
               M_LOANCALCODPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT :=
                    M_LOANCALCODPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT
                  + MLOANIA_OD_INT_AMT_RND (W_BRN_INDEX);
            ELSE
               M_LOANCALCODPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT :=
      ...
      -- Note: The Following Portion of Code is put under condition for Not to Post NPA Accounts
      --Accrual Amount to Suspense at EoD Process but to post at Closure / Reversal.
      IF W_CALL_FROM = '1'
      THEN
         IF MLOANIA_INT_AMT (W_BRN_INDEX) <> 0
         THEN
            IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 1
            THEN
               IF M_LOANCALCNPAPOSTRECORD.EXISTS (W_PROD_CURR_KEY) = TRUE
               THEN
                  M_LOANCALCNPAPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT :=
                       M_LOANCALCNPAPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT
         ...
         THEN
            IF MLOANIA_NPA_STATUS (W_BRN_INDEX) = 1
            THEN
               IF M_LOANCALCODNPAPOSTRECORD.EXISTS (W_PROD_CURR_KEY) = TRUE
               THEN
                  M_LOANCALCODNPAPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT :=
                       M_LOANCALCODNPAPOSTRECORD (W_PROD_CURR_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT
   ...
   PROCEDURE PROCESS_FOR_INT_CALC
   IS
   BEGIN
      W_BRN_INDEX := W_BRN_INDEX + 1;
      MLOANIA_ACNT_NUM (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM;
      MLOANIA_VALUE_DATE (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_VALUE_DATE;

      MLOANIA_MAX_VALUE_DATE (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_MAX_VALUE_DATE;

      MLOANIA_ARR_INT_AMT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ARR_OD_INT_AMT;

      MLOANIA_ACNT_CURR (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_CURR;
      MLOANIA_ACNT_BAL (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_BAL;
      MLOANIA_INT_ON_AMT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_INT_ON_AMT;
      MLOANIA_OD_PORTION (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_OD_PORTION;
      MLOANIA_INT_RATE (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_INT_RATE;
      MLOANIA_SLAB_AMT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_SLAB_AMT;
      MLOANIA_OD_INT_RATE (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_OD_INT_RATE;
      MLOANIA_LIMIT (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_LIMIT;
      MLOANIA_DP (W_BRN_INDEX) :=
         M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_DP;

      PROCESS_FOR_ACTUAL_INTEREST;
   END PROCESS_FOR_INT_CALC;

   PROCEDURE SET_TRAN_KEY_VALUES (W_AC_PREV_BRN_CODE IN NUMBER)
   IS
   BEGIN
      ...
      PKG_AUTOPOST.PV_TRAN_KEY.TRAN_DATE_OF_TRAN :=
         PKG_PB_GLOBAL_EOD_SOD.T_INSTALL (V_GLOB_ENTITY_NUM).MN_CURR_BUSINESS_DATE;
   ...
   BEGIN
      ...
      WHILE W_INDEX_KEY IS NOT NULL
      LOOP
         IF M_LOANCALCPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT <>
               0
         THEN
            ...
            IF MLOAN_PROD_CURR_GL.EXISTS (W_INDEX_KEY)
            THEN
               IF TRIM (MLOAN_PROD_CURR_GL (W_INDEX_KEY).LNPRDAC_INT_ACCR_GL)
                     IS NULL
               THEN
                  W_ERR_MSG :=
                        'Accounting Parameter Not Defined(ACCRUAL) - Prod Code = '
                     || W_PROD_CODE
                     || '  '
                     || ' Curr Code = '
                     || W_CURR_CODE;
               ...
               ELSE
                  W_ACCR_DB_CR_GLCODE :=
                     MLOAN_PROD_CURR_GL (W_INDEX_KEY).LNPRDAC_INT_ACCR_GL;
                  MOVE_POST_ARRAY_VALUES (
                     W_ACCR_DB_CR,
                     W_CURR_CODE,
                     ABS (
                        M_LOANCALCPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT),
                     W_PREV_BRN_CODE,
                     W_ACCR_DB_CR_GLCODE,
                     W_PROD_CODE);
               END IF;
               ...
               IF TRIM (
                     MLOAN_PROD_CURR_GL (W_INDEX_KEY).LNPRDAC_INT_INCOME_GL)
                     IS NULL
               THEN
                  W_ERR_MSG :=
                        'Accounting Parameter Not Defined(INCOME) - Prod Code = '
                     || W_PROD_CODE
                     || '  '
                     || ' Curr Code = '
                     || W_CURR_CODE;
            ...
            ELSE
               W_ERR_MSG :=
                     'Accounting Parameter Not Defined - Prod Code = '
                  || W_PROD_CODE
                  || '  '
                  || ' Curr Code = '
                  || W_CURR_CODE;
      ...
      WHILE W_INDEX_KEY IS NOT NULL
      LOOP
         IF M_LOANCALCODPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT <>
               0
         THEN
            W_PROD_CODE := SUBSTR (W_INDEX_KEY, 1, 4);
            W_CURR_CODE := SUBSTR (W_INDEX_KEY, 5, 3);
            ...
            THEN
               ...
               THEN
                  W_ERR_MSG :=
                        'Accounting Parameter Not Defined(OVERDUE) - Prod Code = '
                     || W_PROD_CODE
                     || '  '
                     || ' Curr Code = '
                     || W_CURR_CODE;
               ...
               ELSE
                  ...
                  MOVE_POST_ARRAY_VALUES (
                     ...
                     W_CURR_CODE,
                     ABS (
                        M_LOANCALCODPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT),
                     W_PREV_BRN_CODE,
                     W_ACCR_DB_CR_GLCODE,
                     W_PROD_CODE);
               END IF;
      ...
      WHILE W_INDEX_KEY IS NOT NULL
      LOOP
         IF M_LOANCALCNPAPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT <>
               0
         THEN
            W_PROD_CODE := SUBSTR (W_INDEX_KEY, 1, 4);
            W_CURR_CODE := SUBSTR (W_INDEX_KEY, 5, 3);
            ...
            THEN
               ...
               ELSE
                  ...
                  MOVE_POST_ARRAY_VALUES (
                     ...
                     W_CURR_CODE,
                     ABS (
                        M_LOANCALCNPAPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT),
                     W_PREV_BRN_CODE,
                     W_ACCR_DB_CR_GLCODE,
                     W_PROD_CODE);
               END IF;

               IF TRIM (MLOAN_PROD_CURR_GL (W_INDEX_KEY).LNPRDAC_INT_SUSP_GL)
                     IS NULL
               THEN
                  W_ERR_MSG :=
                        'Accounting Parameter Not Defined(SUSPENSE) - Prod Code = '
                     || W_PROD_CODE
                     || '  '
                     || ' Curr Code = '
                     || W_CURR_CODE;
               ...
               ELSE
                  W_INCOME_DB_CR_GLCODE :=
                     MLOAN_PROD_CURR_GL (W_INDEX_KEY).LNPRDAC_INT_SUSP_GL;
                  MOVE_POST_ARRAY_VALUES (
                     W_INCOME_DB_CR,
                     W_CURR_CODE,
                     ABS (
      ...
      WHILE W_INDEX_KEY IS NOT NULL
      LOOP
         IF M_LOANCALCODNPAPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT <>
               0
         THEN
            W_PROD_CODE := SUBSTR (W_INDEX_KEY, 1, 4);
            W_CURR_CODE := SUBSTR (W_INDEX_KEY, 5, 3);
            ...
            THEN
               ...
               ELSE
                  ...
                  MOVE_POST_ARRAY_VALUES (
                     ...
                     W_CURR_CODE,
                     ABS (
                        M_LOANCALCODNPAPOSTRECORD (W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT),
                     W_PREV_BRN_CODE,
                     W_ACCR_DB_CR_GLCODE,
                     W_PROD_CODE);
               END IF;
   ...
   PROCEDURE POST_BRN_WISE_TRANSACTION (W_AC_PREV_BRN_CODE IN NUMBER)
   IS
   BEGIN
   ...
   PROCEDURE INSERT_ROW_INTO_LOANIADTL
   IS
   BEGIN
      IF MLOANIA_ACNT_NUM.COUNT > 0
      THEN
         FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT
            INSERT INTO LOANIADTL (LOANIADTL_ENTITY_NUM,
                                   LOANIADTL_BRN_CODE,
                                   LOANIADTL_ACNT_NUM,
                 ...
                 VALUES (V_GLOB_ENTITY_NUM,
                         W_AC_PREV_BRN_CODE,
                         MLOANIADTL_ACNT_NUM (M_INDEX),
                         MLOANIADTL_VALUE_DATE (M_INDEX),
                         MLOANIADTL_ACCRUAL_DATE (W_BRN_INDEX), --W_PROCESS_DATE,
                         MLOANIADTL_SL_NUM (M_INDEX),
                         MLOANIADTL_INT_RATE (M_INDEX),
                         MLOANIADTL_UPTO_AMT (M_INDEX),
                         MLOANIADTL_INT_AMT (M_INDEX),
                         MLOANIADTL_INT_AMT_RND (M_INDEX));
      END IF;
   END INSERT_ROW_INTO_LOANIADTL;



   PROCEDURE INSERT_ROW_INTO_LOANIA
   IS
   BEGIN
      IF MLOANIA_ACNT_NUM.COUNT > 0
      THEN
         FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT
            INSERT INTO LOANIA (LOANIA_ENTITY_NUM,
                                LOANIA_BRN_CODE,
                                LOANIA_ACNT_NUM,
                                LOANIA_VALUE_DATE,
                                LOANIA_ACCRUAL_DATE,
                 ...
                 VALUES (V_GLOB_ENTITY_NUM,
                         ...
                         MLOANIA_MAX_VALUE_DATE (W_BRN_INDEX), --W_PROCESS_DATE,
   ...
   PROCEDURE INSERT_ROW_INTO_LOANIAMRR
   IS
   BEGIN
      IF MLOANIA_ACNT_NUM.COUNT > 0
      THEN
         FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT
			...
			FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT           --added in 13-APR-2017
            INSERT INTO LOANIAMRRDTL LL (LOANIAMRRDTL_ENTITY_NUM,
   ...
   EXCEPTION
      ...
      THEN
         W_ERR_MSG :=
            'Error in Updating LNSUSPLED (Creation of Suspense Ledger)';
         RAISE E_USEREXCEP;
         ...
         UPDATE LNSUSPBAL
            SET LNSUSPBAL_SUSP_BAL =
                ...
                LNSUSPBAL_SUSP_CR_SUM =
                   LNSUSPBAL_SUSP_CR_SUM + ABS (W_NPA_ACT_INT_AMT),
                LNSUSPBAL_INT_BAL =
      ...
      THEN
        ...
        <<ADD_LNSUSPBAL>>
         ...
         EXCEPTION
            ...
            THEN
               W_ERR_MSG :=
                  'Error in Updating LNSUSPBAL (Creation of Suspense Balance)';
               RAISE E_USEREXCEP;
         END ADD_LNSUSPBAL;
   END UPDATE_LNSUSPBAL;

   PROCEDURE GET_ACWISE_SUSP_INT
   IS
   BEGIN
      W_NPA_TOT_INT_AMT := 0;
      W_NPA_ACNT_NUM := 0;
      ...
      FOR J IN 1 .. MLOANIA_ACNT_NUM.COUNT
      LOOP
         IF MLOANIA_NPA_STATUS (J) = 1 AND W_NPA_ACNT_NUM = 0
         THEN
            W_NPA_ACNT_NUM := MLOANIA_ACNT_NUM (J);
            W_NPA_TOT_INT_AMT :=
                 NVL (MLOANIA_INT_AMT_RND (J), 0)
               + NVL (MLOANIA_OD_INT_AMT_RND (J), 0);
            W_CURR_CODE := MLOANIA_ACNT_CURR (J);
            NPAINDEX := 1;
         ELSIF     MLOANIA_NPA_STATUS (J) = 1
               AND W_NPA_ACNT_NUM <> MLOANIA_ACNT_NUM (J)
               AND W_NPA_ACNT_NUM <> 0
         THEN
      ...
      THEN
         FOR K IN 1 .. NPAINDEX
         LOOP
            IF M_ACWISE_NPA_INT (K).M_NPA_TOT_INT_AMT <> 0
            THEN
   ...
   --Note: Update Account level Accrual Date.. This Procedure will only call for Closure / Reversal.
   -- (Still May Be Some Problems are here)
   PROCEDURE UPDATE_ACNTS_INT_ACCR_UPTO
   IS
   BEGIN
      IF MLOANIA_ACNT_NUM.COUNT > 0
      THEN
         FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT
            UPDATE LOANACNTS L
               SET L.LNACNT_INT_ACCR_UPTO = MLOANIA_MAX_VALUE_DATE (M_INDEX) --,    --W_PROCESS_DATE,
         ...
         FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT
            UPDATE ACNTS A
               SET A.ACNTS_INT_ACCR_UPTO = MLOANIA_MAX_VALUE_DATE (M_INDEX) -- W_PROCESS_DATE
   ...
   PROCEDURE PROCESS_FOR_POSTING (W_AC_BRANCH_CODE IN NUMBER)
   IS
   BEGIN
      IF    M_LOANCALCPOSTRECORD.COUNT > 0
         OR M_LOANCALCODPOSTRECORD.COUNT > 0
         OR M_LOANCALCNPAPOSTRECORD.COUNT > 0
         OR M_LOANCALCODNPAPOSTRECORD.COUNT > 0
      THEN
         POST_BRN_WISE_TRANSACTION (W_AC_BRANCH_CODE);
      END IF;

      --Note: IF Call From Reversal / Closure Then it will Update loania otherwise Mirror Table.
      ...
      THEN
         ...
         INSERT_ROW_INTO_LOANIADTL;
      ELSIF W_CALL_FROM = '2'
      THEN
         INSERT_ROW_INTO_LOANIAMRR;
      END IF;


      --Note: When Only Call from Reversal/Closure, the following fields will update.
      IF W_CALL_FROM = '1'
      THEN
         GET_ACWISE_SUSP_INT;
   ...
   PROCEDURE GET_PREVIOUS_INT_AMT_AND_RATE
   IS
          ...
          WHERE     LOANIA_ENTITY_NUM = V_GLOB_ENTITY_NUM
                ...
                AND L.LOANIA_ACCRUAL_DATE =
                       (SELECT MAX (LL.LOANIA_ACCRUAL_DATE)
                          FROM LOANIA LL
                         WHERE     LOANIA_ENTITY_NUM = V_GLOB_ENTITY_NUM
                               AND LL.LOANIA_BRN_CODE =
                                      M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).W_AC_BRN_CODE
                               AND LL.LOANIA_ACNT_NUM =
                                      M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM
                               AND LL.LOANIA_VALUE_DATE =
                                      M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_VALUE_DATE);
   ...
   --- All the data has been found in the main query by Fahim
     ...
     BEGIN
       ...
       EXCEPTION
         WHEN NO_DATA_FOUND THEN
           W_ERR_MSG := 'Account Number not Available in Loanacnts - ' ||
                        V_GLOB_ENTITY_NUM || ' ENTITY ' ||
   ...
   */
   PROCEDURE READ_RTMPLNIA(P_BRN_CODE NUMBER)
   IS
   
   V_SQL VARCHAR2(3000) ;
   
   BEGIN
        V_SQL := ' SELECT RTMPLNIA_RUN_NUMBER,
               RTMPLNIA_ACNT_NUM,
               RTMPLNIA_VALUE_DATE,
         ...
         -- BULK COLLECT INTO M_RTMPLNIA_RECORD
          FROM RTMPLNIA R, ACNTS, LOANACNTS, ASSETCLS, ASSETCD , INSTALL, BANKCD
         WHERE     LNACNT_ENTITY_NUM = ' || V_GLOB_ENTITY_NUM|| '
               AND ACNTS_ENTITY_NUM = ' || V_GLOB_ENTITY_NUM|| '
               ...
               AND LNACNT_INTERNAL_ACNUM = RTMPLNIA_ACNT_NUM
               AND ACNTS_INTERNAL_ACNUM = LNACNT_INTERNAL_ACNUM
               AND R.RTMPLNIA_INSERT_FROM = ''' || V_CALL_FROM_MRR_REVERSAL || '''
               AND RTMPLNIA_BRN_CODE = ACNTS_BRN_CODE
               AND INS_OUR_BANK_CODE = BANKCD_CODE
               ' ;
      IF W_CALL_FROM = 2 THEN 
        V_SQL := V_SQL || 'AND ASSETCD_NONPERF_CAT <> ''3''';
      END IF ;   
   
      IF V_INTERNAL_ACNUM > 0 THEN
        V_SQL := V_SQL || 'AND ASSETCLS_INTERNAL_ACNUM = ' || V_INTERNAL_ACNUM ;
      END IF ;
      
      V_SQL := V_SQL ||  '
       ORDER BY RTMPLNIA_ACNT_NUM, R.RTMPLNIA_VALUE_DATE';
      
      EXECUTE IMMEDIATE V_SQL BULK COLLECT INTO M_RTMPLNIA_RECORD;
      ...
      IF M_RTMPLNIA_RECORD.COUNT > 0
      THEN
         FOR IDX IN 1 .. M_RTMPLNIA_RECORD.COUNT
         LOOP
            ...
            IF     W_AC_PREV_BRN_CODE <>
                      M_RTMPLNIA_RECORD (IDX).W_AC_BRN_CODE
               AND W_AC_PREV_BRN_CODE <> 0
            THEN
               PROCESS_FOR_POSTING (W_AC_PREV_BRN_CODE);
            END IF;


            IF M_RTMPLNIA_RECORD (IDX).W_INT_ACCR_UPTO_DATE IS NOT NULL
            THEN
               IF M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_VALUE_DATE <=
                     M_RTMPLNIA_RECORD (IDX).W_INT_ACCR_UPTO_DATE
               THEN
                  GET_PREVIOUS_INT_AMT_AND_RATE;
               END IF;
            END IF;

            IF (   M_RTMPLNIA_RECORD (IDX).W_INT_ACCR_UPTO_DATE IS NULL
                OR M_RTMPLNIA_RECORD (IDX).W_INT_ACCR_UPTO_DATE <
                      W_PROCESS_DATE)
            THEN
               PROCESS_FOR_INT_CALC;
            END IF;

            W_AC_PREV_BRN_CODE := M_RTMPLNIA_RECORD (IDX).W_AC_BRN_CODE;
         END LOOP;

         IF W_AC_PREV_BRN_CODE <> 0
         THEN
            PROCESS_FOR_POSTING (W_AC_PREV_BRN_CODE);
         END IF;
      END IF;
   ...
   PROCEDURE PROCESS_FROM_RTMPLNIA(P_BRN_CODE NUMBER )
   IS
   BEGIN
     <<READRTMPLNIA>>
      BEGIN
   ...
   BEGIN
      ...
      PKG_PB_GLOBAL_EOD_SOD.SET_INSTALL_CONFIG (V_ENTITY_NUM);

      V_INTERNAL_ACNUM := P_INTERNAL_ACNUM ;
      ...
      W_CALL_FROM := P_CALL_FROM;
      IF W_CALL_FROM = '1' THEN -- 1 = REVERSAL, 2 = MIRROR POSTING
        V_CALL_FROM_MRR_REVERSAL := 'R';
      ELSE
        V_CALL_FROM_MRR_REVERSAL := 'M';
      END IF ;
