CREATE OR REPLACE PACKAGE PKG_LOANDAILYACCRPOST_FOR_CLS IS
  PROCEDURE SP_LOANACCRPOST_FOR_CLS(V_ENTITY_NUM IN NUMBER, P_INTERNAL_ACNUM IN NUMBER, P_BATCH_NUM OUT NUMBER);
END PKG_LOANDAILYACCRPOST_FOR_CLS;
/
CREATE OR REPLACE PACKAGE BODY PKG_LOANDAILYACCRPOST_FOR_CLS IS
  /*
...
-- Avinash-SONALI-17AUG2012 End
  ...
  PROCEDURE INSERT_INTO_LOANIADTL IS
  BEGIN
    INSERT INTO LOANIADTL LL
      (LOANIADTL_ENTITY_NUM,
       LOANIADTL_BRN_CODE,
       LOANIADTL_ACNT_NUM,
      ...
      (SELECT PKG_ENTITY.FN_GET_ENTITY_CODE,
              ...
              RTMPLNIADTL_INT_AMT,
              SIGN(RTMPLNIADTL_INT_AMT_RND) *
              ABS(SP_GETFORMAT(PKG_ENTITY.FN_GET_ENTITY_CODE,
                               W_AC_CURR_CODE,
                               RTMPLNIADTL_INT_AMT_RND,
                               1))
         FROM RTMPLNIADTL L
        WHERE L.RTMPLNIADTL_RUN_NUMBER = W_RUN_NUMBER
          AND L.RTMPLNIADTL_ACNT_NUM = M_RTMPLNIA_RECORD(W_ARRAY_PROCESS_SL)
             .RTMPLNIA_ACNT_NUM
          AND L.RTMPLNIADTL_VALUE_DATE = M_RTMPLNIA_RECORD(W_ARRAY_PROCESS_SL)
             .RTMPLNIA_VALUE_DATE);
  END INSERT_INTO_LOANIADTL;
  ...
  BEGIN
   ...
   W_NPA_INT_POST_AMOUNT_RND:= 0;
     /* W_NPA_INT_POST_AMOUNT := W_NPA_INT_POST_AMOUNT +
      ...
      --02-12-2008-beg
      W_NPA_INT_POST_AMOUNT_RND := SP_GETFORMAT(PKG_ENTITY.FN_GET_ENTITY_CODE,
                                                W_AC_CURR_CODE,
  ...
  BEGIN
      ...
      ELSE
        ...
        W_CRDB_FLG := 'C';
      END IF;
      W_CBD_TIME := W_PROCESS_DATE || ' ' ||
                    PKG_PB_GLOBAL.FN_GET_CURR_BUS_TIME(PKG_ENTITY.FN_GET_ENTITY_CODE);
    ...
    BEGIN
      SELECT L.LNACNT_INT_ACCR_UPTO,
       ...
       WHERE LNACNT_ENTITY_NUM = PKG_ENTITY.FN_GET_ENTITY_CODE
         ...
         AND L.LNACNT_INTERNAL_ACNUM = M_RTMPLNIA_RECORD(W_ARRAY_PROCESS_SL)
            .RTMPLNIA_ACNT_NUM
  ...
  BEGIN
    SELECT RTMPLNIA_RUN_NUMBER,
          ...
          FROM RTMPLNIA R
         WHERE     R.RTMPLNIA_ACNT_NUM = V_INTERNAL_ACNUM
               AND R.RTMPLNIA_INSERT_FROM = 'C'
               --AND R.RTMPLNIA_ACCRUAL_DATE = W_CBD
  ...
  PROCEDURE PROCESS_FROM_RTMPLNIA IS
  BEGIN
    <<READRTMPLNIA>>
    BEGIN
  ...
  PROCEDURE SP_LOANACCRPOST_FOR_CLS(V_ENTITY_NUM IN NUMBER, P_INTERNAL_ACNUM IN NUMBER, P_BATCH_NUM OUT NUMBER) IS
  BEGIN
	...
	W_CBD := PKG_PB_GLOBAL.FN_GET_CURR_BUS_DATE (V_ENTITY_NUM);
	
    -- Avinash-SONALI-17AUG2012 (begin)
    W_USER_ID          := PKG_EODSOD_FLAGS.PV_USER_ID;
  -- Avinash-SONALI-17AUG2012 (end)
    W_RUN_NUMBER       := PKG_EODSOD_FLAGS.GET_RUN_NUM(PKG_ENTITY.FN_GET_ENTITY_CODE);
    W_AC_PREV_BRN_CODE := 0;
    PROCESS_FROM_RTMPLNIA;
    P_BATCH_NUM := W_BATCH_NUM; -- ADDED BY BIBHU ON 29-11-2012
  END SP_LOANACCRPOST_FOR_CLS;
END PKG_LOANDAILYACCRPOST_FOR_CLS;
/
