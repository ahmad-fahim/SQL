/*<TOAD_FILE_CHUNK>*/
CREATE OR REPLACE PROCEDURE SP_SBS3_DATA_QUERY (P_FROM_DATE       DATE,
                                                P_DATA        OUT CLOB)
IS
   V_DATA       CLOB;
   V_FIN_YEAR   NUMBER;
BEGIN
   ...
   V_DATA :=
         'SELECT  TO_CHAR (:V_TO_DATE, ''DDMMYYYY'')
      ...
      || ' T
        WHERE  A.TRANADV_ENTITY_NUM = T.TRAN_ENTITY_NUM
                ...
                || TRIM(TO_CHAR(NVL(( ABS(FN_GET_ASON_ACBAL(:V_ENTITY_NUM, A.ACNTS_INTERNAL_ACNUM , A.ACNTS_CURR_CODE, :V_TO_DATE, :V_CBD, ''0''))  ), 0),''0000000000000000000000.00''))
          TOTAL_DATA
  FROM PRODUCTS P,
       IACLINK I,
       ACNTS A,
 ...
 WHERE     P.PRODUCT_CODE = I.IACLINK_PROD_CODE
       ...
       AND  ABS(FN_GET_ASON_ACBAL(:V_ENTITY_NUM, A.ACNTS_INTERNAL_ACNUM , A.ACNTS_CURR_CODE, :V_TO_DATE, :V_CBD, ''0'')) <> 0
       ORDER BY A.ACNTS_BRN_CODE';
...
/*<TOAD_FILE_CHUNK>*/
CREATE OR REPLACE PACKAGE BODY PKG_SBS3_DATA
IS
